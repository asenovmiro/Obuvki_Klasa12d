...
namespace Obuvki_Klasa.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _userManager.AddToRoleAsync(client, "User");   //!!! dobavqne na rolya User

                    var clientId = await _userManager.GetUserIdAsync(client);
                    var code = await _userManager.GenerateEmailConfirmationTokenAsync(client);
                    code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                    var callbackUrl = Url.Page(
                        "/Account/ConfirmEmail",
                        pageHandler: null,
                        values: new { area = "Identity", userId = clientId, code = code, returnUrl = returnUrl },
